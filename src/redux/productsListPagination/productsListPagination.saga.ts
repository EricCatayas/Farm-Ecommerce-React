...
} from "./productsListPagination.action";
...
export function* fetchFilteredProductsAsync(
  action: PayloadAction<ProductQueryParams>
): SagaIterator {
  try {
    const productsService = new ProductsService();
...
export function* fetchSearchProductsAsync(
  action: PayloadAction<string>
): SagaIterator {
  try {
    const productsService = new ProductsService();
    const data = yield call(
      productsService.fetchSearchProductsAsync,
      action.payload
    );
    //LOG
...
export function* onFetchSearchProducts(): SagaIterator {
  yield takeLatest(
